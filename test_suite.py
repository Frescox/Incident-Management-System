'''
Instalar dependencias si es necesario para ejecutar las  (ya vienen en el requirements)
pip install pytest
...
# Se ocupa tener el sistema de incidencias corriendo en el puerto 8080 para que locust funcione y no falle :C, con las demas pruebas no hay problema, solo es esa, la 3.9
# python run.py
...
def test_usuario_model_tdd():
    ...
    assert user.nombre != test_data["nombre"]
...
def test_escalabilidad(app):
    """Prueba de escalabilidad con manejo de sesiones por thread"""
    with app.app_context():
        ...
        def create_incident(app, user_id):
            with app.app_context():
                # Nueva sesión para cada thread
        ...
        for i in range(100):
            t = Thread(target=create_incident, args=(app, i+1))
            threads.append(t)
            t.start()
        ...
        print(f"Tiempo total para 100 incidencias concurrentes: {total_time}")
...
# 3.9 Herramientas de prueba y automatización
